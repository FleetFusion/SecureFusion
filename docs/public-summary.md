...
- **A versioning scheme.** `SF1`, `SF2`, etc. As the standard evolves (per-frame hashing, hardware-signed capture, qualified timestamps), older anchors remain verifiable forever.
...
Anchoring to a public chain is the only way to make tamper-evidence independent of the vendor. A private ledger run by a fleet platform proves nothing, since the platform could rewrite it. A trusted third-party timestamping service is better, but still requires trusting that third party. A public blockchain anchor can be verified by anyone, in any country, decades from now, without anyone's permission or cooperation.

XRP Ledger and Bitcoin are chosen specifically. XRPL provides fast, cheap, predictable confirmation with rich on-chain memo capacity, serving as the operational layer. Bitcoin provides the deepest archival guarantees and the strongest legal precedent, serving as the durable layer. Together they give SecureFusion both immediacy and longevity.
